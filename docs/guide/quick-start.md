...
1. **系统配置**
   ...
   进入 **系统配置** 页面，配置 **TMDB API Key**（用于影视搜索元数据）。如需科学上网，配置 **网络代理**（HTTP/SOCKS5）。

2. **115 网盘**

   进入 **115 助手** 页面，添加 115 账号 Cookie，配置默认转存目录和离线下载目录。

3. **Telegram Bot**

   在 Emby 助手的 Bot 配置标签页中填入 Bot Token（从 [@BotFather](https://t.me/BotFather) 获取），配置 MTProto Session（用于频道监控），Session 登录支持验证码、二步验证、QR 码三种方式。

4. **HDHive 配置** <Badge type="tip" text="可选" />

   进入 **HDHive 用户** 页面，添加 HDHive API Key，系统会自动进行负载均衡。

5. **频道监控与转存** <Badge type="tip" text="可选" />

   在 **监控转发** 中添加源频道和目标频道，在 **监控转存** 中配置需要自动转存的频道。
